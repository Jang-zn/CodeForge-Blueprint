...
아래는 기존 코드베이스를 분석한 자동 스캔 결과입니다(디렉토리 구조 + 주요 파일 발췌). 이 스캔 결과를 출발점으로 삼되, **소스 파일을 직접 읽어 더 깊이 분석하세요**. 이 코드베이스를 처음 접하는 신규 팀원이 프로젝트 전체를 이해할 수 있도록, 두 가지를 겸한 상세 문서를 작성하세요:
(1) PRD — 이 제품이 무엇이고 무엇을 해결하는지
(2) 개발자 온보딩 가이드 — 코드베이스 구조, 실행 방법, 컨벤션

## 작성 원칙
- 구현된 기능만 기술하세요. 미구현 기능은 추측하지 마세요. README에만 언급된 것은 [TBD] 표시하세요.
- 각 섹션을 충분한 깊이로 서술하세요. 간략한 한 줄 요약은 금지합니다. 코드에서 관찰한 구체적인 증거를 들어 서술하세요.
- 스캔 결과에 없는 세부 내용은 직접 파일을 읽어 확인하세요. 핵심 소스 파일(라우트, 모델, 주요 로직)은 반드시 직접 읽으세요.
- 코드베이스 분석 결과에 없는 정보는 절대 만들어내지 마세요.
